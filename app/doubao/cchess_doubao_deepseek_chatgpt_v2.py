# 优化版：动态学习率 + 快速收敛
# - 比v0版增加自动调节下降梯度
# - 增强v1版的HybridXiangqiModel实现，使用gelu代替relu，增加编码宽度和注意力头数，下面是主要增强分析：
# 1. 模型容量更大，表达能力更强
# 嵌入维度：版本v2的 d_model=128（版本B为64），能表示更丰富的特征。
# 注意力头数：版本v2有8个头（版本B为4个），可捕获更多子空间交互。
# Transformer层数：版本v2有4层编码器（版本B为2层），能建模更深的序列依赖。
# 前馈网络维度：版本v2的 dim_feedforward=512（版本B为128），显著增强非线性变换能力。
# 动作预测头：版本v2使用两层MLP（256→90），相比版本B的单层线性变换，具备更强的拟合能力。
# 2. 激活函数更优
# 版本v2采用 GELU，相比ReLU更平滑、非单调，常能提升深度网络的收敛速度和泛化性能。
# 3. 位置编码与输入布局合理
# 版本v2使用 batch_first=True 且对序列维度做平均池化后再预测，逻辑清晰，与Transformer的常见用法一致。

# - 代码框架由doubao提供
# - 参数动态调节由deepseek v4 flash提供
# - v2版新的HybridXiangqiModel由chatgpt提供
# - 不限制轮次训练，随时停，随时继续
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        # self.pe: (max_len, 1, d_model),  x: (batch, seq_len, d_model)
        # 转置为 (1, seq_len, d_model) 以匹配 batch_first 的维度顺序
...
class HybridXiangqiModel(nn.Module):
    ...
    def __init__(self, d_model=128, nhead=8, num_layers=4):
        ...
        self.cnn = nn.Sequential(
            ...
            nn.BatchNorm2d(32),  # 加这一行
            nn.GELU(),
            nn.Conv2d(32, 64, 3, padding=1),
            nn.BatchNorm2d(64),  # 加这一行
            nn.GELU(),
            nn.Conv2d(64, d_model, 3, padding=1),
            nn.BatchNorm2d(d_model),  # 加这一行
            nn.GELU(),
    ...
    #     print(f"input  | mean={x.mean().item():.4f} std={x.std().item():.4f} min={x.min().item():.2f} max={x.max().item():.2f}")
        
    #     x = self.cnn(x)
    #     print(f"after CNN | mean={x.mean().item():.4f} std={x.std().item():.4f} min={x.min().item():.2f} max={x.max().item():.2f}")
        
    #     x = x.flatten(2).transpose(1, 2)  # [B, 90, d_model]
    #     print(f"after flatten | mean={x.mean().item():.4f} std={x.std().item():.4f}")
        
    #     x = self.pos_enc(x)
    #     print(f"after pos_enc | mean={x.mean().item():.4f} std={x.std().item():.4f}")
        
    #     x = self.transformer(x)
    #     print(f"after transformer | mean={x.mean().item():.4f} std={x.std().item():.4f}")
        
    #     x = x.mean(dim=1)
    #     print(f"after mean | mean={x.mean().item():.4f} std={x.std().item():.4f}")
        
    #     pred_from = self.from_head(x)
    #     pred_to = self.to_head(x)
    #     print(f"pred_from | min={pred_from.min().item():.2f} max={pred_from.max().item():.2f}")
    #     return pred_from, pred_to
...
if __name__ == "__main__":
    ...
    if os.path.exists(CACHE_PATH):
        try:
            ...
            if (
                ...
                raise ValueError(
                    f"旧缓存格式(len={len(data) if hasattr(data,'__len__') else '?'})"
                )
            # 确保内存中为float32
            ...
            print(
                f"   紧凑存储: {boards.nbytes/1e9:.1f}GB (vs 全状态 {(boards.nbytes * IN_CHANNELS * 9 * 10 / (9*10))/1e9:.1f}GB)"
            )
            ...
            _, boards, histories, turns, yf, yt, mf, mt = iccs_games_to_dataset_compact(
                games, max_games=len(games)
            )
            mf = np.asarray(mf, dtype=np.float32)
            mt = np.asarray(mt, dtype=np.float32)
            save_cache(
                ("CACHE_v2", boards, histories, turns, yf, yt, mf, mt), CACHE_PATH
            )
    ...
    print(
        f"  from标签匹配mask: {n_check - bad_from}/{n_check} ✅"
        if bad_from == 0
        else f"  ❌ from标签非法: {bad_from}/{n_check}"
    )
    print(
        f"  to标签匹配mask:   {n_check - bad_to}/{n_check} ✅"
        if bad_to == 0
        else f"  ❌ to标签非法:   {bad_to}/{n_check}"
    )
    print(f"  全非法from mask: {all_inf_from}  全非法to mask: {all_inf_to}")
    if bad_from > 0 or bad_to > 0:
        print(
            "  ⚠️  标签与mask不匹配，请检查 iccs_move_to_pos 行反转 或 FEN_PIECE 别名"
        )
        # 打印第一个出错样本的详情
        for i in range(min(5, n_check)):
            if mf[i, yf[i]] != 0.0 or mt[i, yt[i]] != 0.0:
                ...
                print(f"\n  样本{i}: turn={turns[i]}")
                print(
                    f"    from=({fc},{fr}) mask_val={mf[i, yf[i]]}  board_val={boards[i, fc, fr]}"
                )
                print(
                    f"    to=({tc},{tr})   mask_val={mt[i, yt[i]]}  board_val={boards[i, tc, tr]}"
                )
    ...
    # 🔍 迷你前向传播测试
    print("\n🔍 迷你前向传播测试...")
    test_dataset = CompactXiangqiDataset(
        boards[:16], histories[:16], turns[:16], yf[:16], yt[:16], mf[:16], mt[:16]
    )
    test_loader = DataLoader(test_dataset, batch_size=16, shuffle=False, num_workers=0)
    test_model = HybridXiangqiModel()
    test_model.apply(_init_weights)

    test_model.eval()
    with torch.no_grad():
        for bx, bf, bt, bm_f, bm_t in test_loader:
            pf_raw, pt_raw = test_model(bx.float())
            # 添加合法走法掩码
            pf = pf_raw + bm_f
            pt = pt_raw + bm_t

            # 打印原始和掩码输出
            print(f"  原始(掩码) pred_f: range=[{pf_raw.min():.4f}({pf.min():.4f}), {pf_raw.max():.4f}({pf.max():.4f})]")
            print(f"  原始(掩码) pred_t: range=[{pt_raw.min():.4f}({pt.min():.4f}), {pt_raw.max():.4f}({pt.max():.4f})]")


            # # 打印加掩码后的输出（用于训练/测试）
            # print(f"  掩码后 pred_f: range=[{pf.min():.4f}, {pf.max():.4f}]")
            # print(f"  掩码后 pred_t: range=[{pt.min():.4f}, {pt.max():.4f}]")
    ...
    if os.path.exists(CHECKPOINT_PATH):
        try:
            ...
            print("⚠️  断点不兼容（通道数从12→26），删除旧断点重新训练")
            print(f"   原因: {e}")
